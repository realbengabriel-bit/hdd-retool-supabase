...
select
  ...
  coalesce(dt.label, initcap(replace(d.document_type_code, '_', ' ')), 'Other document') as document_type_label,
  coalesce(d.document_category, dt.category, 'other') as document_category,
  d.status,
  case
  ...
  if to_regclass('public.v_retool_workflow_detail_core') is not null then
    execute $view$
      ...
      select
        ...
        coalesce(dt.label, initcap(replace(coalesce(d.document_type_code, 'other'), '_', ' ')), 'Other document') as document_type_label,
        coalesce(d.document_category, dt.category, 'other') as document_category,
...
select
  ...
  coalesce(sdt.label, initcap(replace(ij.suggested_document_type_code, '_', ' '))) as suggested_document_type_label,
  ij.job_status,
  case
...
select
  ...
  coalesce(sdt.label, initcap(replace(rq.suggested_document_type, '_', ' '))) as suggested_document_type_label,
  rq.final_document_type,
  coalesce(fdt.label, initcap(replace(rq.final_document_type, '_', ' '))) as final_document_type_label,
