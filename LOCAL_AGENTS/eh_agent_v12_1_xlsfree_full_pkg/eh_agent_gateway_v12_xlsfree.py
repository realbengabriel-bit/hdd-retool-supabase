...
SERVICE_NAME = "eh_agent_gateway_v12_xlsfree"
...
EH_FILL_SCRIPT_PATH = os.getenv("EH_FILL_SCRIPT_PATH", r"C:\EH_AGENT\eh_agent_v12_xlsfree_pkg\eh_enterhungary_assistant_v12_payload_first.py")
...
app = FastAPI(
    title="EH Agent Gateway v11 P02-first",
    version=SERVICE_VERSION,
    description="P02-first EnterHungary/OIF agent wrapper. Legacy is search-only by design.",
)
...
def infer_application_type_for_agent(data: Dict[str, Any]) -> str:
    ...
    blob = " ".join(str(x or "") for x in [
        ...
        workflow.get("application_type"), workflow.get("request_code"), bmh.get("application_type"),
        data.get("application_type"), data.get("application_type_code"), data.get("app_type"),
    ]).lower()
    if "foglalkoztatasi_9_6" in blob or "foglalkoztatas" in blob or "employment" in blob:
        return "foglalkoztatasi_9_6"
    if "vendeg_9_7" in blob or "guest_worker_extension" in blob or "guest worker extension" in blob:
        return "vendeg_9_7"
    if "nemzeti" in blob or "national" in blob:
        return "nemzeti"
